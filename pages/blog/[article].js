              ...
              <div className={style.info}>
                <div className={style.authorDetails}>
                  <div className={style.imgContainer}>
                    <Link href={`/fam/${article_author.name}`}>
                      <a>
                        <img src={article_author.avatar.url} alt="" />
                      </a>
                    </Link>
                  </div>
                  <div className={style.nameAndDate}>
                    <Link href={`/fam/${article_author.name}`}>
                      <a className={style.author}>{article_author.name}</a>
                    </Link>
                ...
                <div className={style.shareIcons}>
                  <a
                    ...
                    href={`https://www.linkedin.com/shareArticle?mini=true&url=https%3A%2F%2Fwww.flugelhorn.co%2Fblog%2F${shareUrl}&title=${shareUrl}&source=Flugelhorn?`}
                  >
                  ...
                  <a
                    target="_blank"
                    href={`https://twitter.com/intent/tweet?url=https%3A%2F%2Fwww.flugelhorn.co%2Fblog%2F${shareUrl}&hashtags=flugelhorn`}
                  >
                  ...
                  <a
                    href={`https://www.facebook.com/sharer.php?u=http%3A%2F%2Fwww.flugelhorn.co%2Fblog%2F${shareUrl}`}
                  >
                    <svg className={style.icon} viewBox="0 0 25 25">
...
export async function getStaticPaths() {
  ...
  const paths = res.map((article) => {
    ...
    return {
      params: { article: name.replace(new RegExp(" ", "g"), "%20") },
    };
  });
